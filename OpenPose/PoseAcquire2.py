...
class Pose():
    ...
    def __init__(self, mode="COCO", threadCount=2, inWidth = 120, inHeight = 120, threshold = 0.1):
        ...
        #init neural networks openpose e dei semafori
        self.net = [None] * self.threadCount
        self.semaphores = [None] * self.threadCount
        self.output = None
        for i in range(self.threadCount):
            self.net[i] = cv2.dnn.readNetFromCaffe(protoFile, weightsFile)
            self.semaphores[i] = threading.Semaphore(1)
    
    #return last pose and start thread for acquire new pose from current frame
    def getPose(self, frame):
        for i in range(self.threadCount):
            is_acquire = self.semaphores[i].acquire(blocking=False)
            if is_acquire:
                frameInput = frame.copy()
                thread = threading.Thread(target=self.getPoseAsync, args=(i, frameInput, is_acquire,))
                thread.start()
                break
    ...
    def getPoseAsync(self, i, frameInput, is_acquire):
        #startTime = time.time()
        with contextlib.ExitStack() as stack:
            stack.callback(self.semaphores[i].release)

            #print("start thread " + str(i))
            inpBlob = cv2.dnn.blobFromImage(frameInput, 1.0 / 255, (self.inWidth, self.inHeight), (0, 0, 0), swapRB=False, crop=False)
            net = self.net[i]
            ...
            self.lock.acquire()
            try:
                self.output = self.getKeyPoints(frameInput, output)
            finally:
                self.lock.release()
